...
<xsl:stylesheet version="1.0"
  ...
  exclude-result-prefixes="marc items">
    <xsl:import href="http://www.sekls.net/stylesheets/MARC21slimUtils4.xsl"/>
				  ...
				  <xsl:attribute name="href"><xsl:value-of select="marc:subfield[@code='u']"/></xsl:attribute>
				   ...
				   <xsl:choose>
					...
					<xsl:otherwise>
                        ...
                        <!-- Add an icon if 007 is populated, based on pos00, pos01, pos05 | -->
                        <xsl:if test="marc:controlfield[@tag=007]!=''">
                            <div class="format_icon">
                            <xsl:choose>
                                <xsl:when test="marc:controlfield[@tag=007][substring(text(),1,1)='v'][substring(text(),2,1)='f'][substring(text(),5,1)='b']"><img src="http://sekls.net/Pictures/VHS_Icon.png" title="VHS"></img></xsl:when>
                                <xsl:when test="marc:controlfield[@tag=007][substring(text(),1,1)='v'][substring(text(),2,1)='d'][substring(text(),5,1)='v']"><img src="http://sekls.net/Pictures/DVD_Icon.png" title="DVD"></img></xsl:when>
                                <xsl:when test="marc:controlfield[@tag=007][substring(text(),1,1)='v'][substring(text(),2,1)='d'][substring(text(),5,1)='s']"><img src="http://sekls.net/Pictures/BluRay_Icon.png" title="Bluray"></img></xsl:when>
                            </xsl:choose>
                            </div>
                        </xsl:if>
