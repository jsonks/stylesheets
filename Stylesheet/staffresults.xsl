                ...
                <xsl:for-each select="marc:subfield[contains('bchknps', @code)]">
                    <xsl:choose>
                        ...
                        <xsl:when test="@code='c'">
                            ...
                            <span class="title_sor"><xsl:apply-templates/> </span>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:apply-templates/>
                            <xsl:text> </xsl:text>
		...
		<xsl:for-each select="marc:datafield[@tag=440]">
			<span class="results_summary series"><span class="label">Series: </span>
				...
				<xsl:if test="marc:subfield[@code='a']">
					<xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=se:<xsl:value-of select="marc:subfield[@code='a']"/></xsl:attribute>
					<xsl:call-template name="subfieldSelect">
						<xsl:with-param name="codes">a</xsl:with-param>
					</xsl:call-template>
				</xsl:if>
